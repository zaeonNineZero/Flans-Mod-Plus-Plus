...
public class RenderGrenade extends Render implements IItemRenderer 
{
	...
	public void render(EntityGrenade grenade, double d, double d1, double d2, float f, float f1)
	{
		...
		GL11.glScalef(grenade.type.modelScale, grenade.type.modelScale, grenade.type.modelScale);
        if(grenade.stuck)
        {
	...
	public boolean handleRenderType(ItemStack item, ItemRenderType type) 
	{
		switch(type)
		{
		case EQUIPPED : case EQUIPPED_FIRST_PERSON : return item != null && item.getItem() instanceof ItemGrenade && ((ItemGrenade)item.getItem()).type.model != null && ((ItemGrenade)item.getItem()).type.renderItemModel == true;
		default : break;
		}
		return false;
	}
	...
	public void renderItem(ItemRenderType type, ItemStack item, Object... data) 
	{
		...
		if(item != null && item.getItem() instanceof ItemGrenade)
		{
			...
			if(grenadeType.model != null && grenadeType.renderItemModel == true)
			{
				switch(type)
				{
				case EQUIPPED:
