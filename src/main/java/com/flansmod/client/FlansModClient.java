...
public class FlansModClient extends FlansMod
{
	...
	/** The reload time, for controling other things */
	...
	/** Whether or not the player is dual wielding */
	...
	/** The transition variable for zooming in / out.  Helps smooth the animation. 0 = unscoped, 1 = scoped */
	...
	public static float originalFOV = 75F;
	...
	public static void tick()
	{
		...
		if (playerRecoilPitch > 0)
			playerRecoilPitch *= 0.65F;  //Default is 0.8F
		if (playerRecoilYaw > 0 || playerRecoilYaw < 0)
			playerRecoilYaw *= 0.65F;  //Default is 0.8F
		...
		antiRecoilPitch *= 0.791F;  //Default is 0.8F
		antiRecoilYaw *= 0.791F;  //Default is 0.8F
		...
		//Get required variables from the ItemGun being held
		Item itemInHand = null;
		boolean canSprintShoot = false;
		...
		if(minecraft.thePlayer.isSprinting() && reloadTime <= 0 && !canSprintShoot)
		{
		...
		if (currentScope != null && (itemInHand == null || !(itemInHand instanceof ItemGun && ((ItemGun)itemInHand).type.getCurrentScope(itemstackInHand) == currentScope)))
		{
			...
			scopeTime = (int) Math.max(zoomProgress*13F+1F,4F);
		}
		
		//Unscope and prevent scoping when reloading and/or sprinting
		if (currentScope != null && (reloadTime > 0 || minecraft.thePlayer.isSprinting()))
		{
			currentScope = null;
			minecraft.gameSettings.fovSetting = originalFOV;
			minecraft.gameSettings.mouseSensitivity = originalMouseSensitivity;
		...
		{
			zoomProgress *= 0.56F; //zoomProgress *= 0.66F;
		}
		else
		{
			zoomProgress = 1F - (1F - zoomProgress) * 0.57F; //Default is " zoomProgress = 1F - (1F - zoomProgress) * 0.66; "
		}
