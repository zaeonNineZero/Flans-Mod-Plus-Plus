...
public class TickHandlerClient
{
	    ...
	    if(!event.isCancelable() && event.type == ElementType.HOTBAR)
	    {
				...
				if(stack != null && stack.getItem() instanceof ItemGun)
				{
					...
					for(int n = 0; n < gunType.numAmmoItemsInGun; n++)
					{
						...
						if(bulletStack != null && bulletStack.getItem() != null && bulletStack.getItemDamage() < bulletStack.getMaxDamage())
						{
							...
							mc.fontRenderer.drawString(s, i / 2 + 33 + x, j - 70, 0xffffff);
						...
						if(offHandStack != null && offHandStack.getItem() instanceof ItemGun)
						{
							...
							for(int n = 0; n < offHandGunType.numAmmoItemsInGun; n++)
							{
								...
								if(bulletStack != null && bulletStack.getItem() != null && bulletStack.getItemDamage() < bulletStack.getMaxDamage())
								{
									...
									GL11.glEnable(GL12.GL_RESCALE_NORMAL);
									OpenGlHelper.setLightmapTextureCoords(OpenGlHelper.lightmapTexUnit, 240F, 240F);
									drawSlotInventory(mc.fontRenderer, bulletStack, i / 2 - 32 - x, j - 75);
									x += 16 + mc.fontRenderer.getStringWidth(s);

									//Draw the string
									GL11.glDisable(GL12.GL_RESCALE_NORMAL);
									RenderHelper.disableStandardItemLighting();
									mc.fontRenderer.drawString(s, i / 2 - 16 - x, j - 69, 0x000000);
									mc.fontRenderer.drawString(s, i / 2 - 17 - x, j - 70, 0xffffff);
								}
							}
						}
					}
			...
			if(tickcount > 0)
			{
				...
				GL11.glColor4f(
						...
						FlansMod.hitCrossHairColor[0] * (float)tickcount / 11);
				GL11.glDisable(3008 /* GL_ALPHA_TEST */);
			...
			if(tickcountWounded > 0)
			{
				...
				GL11.glDepthMask(false);
				GL11.glBlendFunc(770, 771);
				GL11.glColor4f(1F, 1F, 1F, 1F * (float)tickcountWounded / 16);
				GL11.glDisable(3008 /* GL_ALPHA_TEST */);
			...
			if(mc.thePlayer.ridingEntity instanceof EntitySeat)
			{
				...
				int healthColour = 0;
				if(healthP >= 100){
					healthColour = 0xffffff;
				} else if (healthP > 80){
					healthColour = 0x00ff00;
				} else if (healthP <= 80 && healthP > 60){
					healthColour = 0xb2ff00;
				} else if (healthP <= 60 && healthP > 40){
					healthColour = 0xffff00;
				} else if (healthP <= 40 && healthP > 20){
					healthColour = 0xee7e00;
				} else if (healthP <= 0) {
					healthColour = 0xdd0000;
				}
				
				int fuelColour = 0;
				if(fuelP > 80){
					fuelColour = 0x00ff00;
				} else if (fuelP <= 80 && fuelP > 60){
					fuelColour = 0xb2ff00;
				} else if (fuelP <= 60 && fuelP > 40){
					fuelColour = 0xffff00;
				} else if (fuelP <= 40 && fuelP > 20){
					fuelColour = 0xf27f00;
				} else if (fuelP <= 20 && fuelP > 0){
					fuelColour = 0xdd0000;
				} else if (fuelP <= 0) {
					fuelColour = 0x4a4a4a;
				}
	...
	public void clientTickStart(Minecraft mc)
	{
		...
		if(FlansMod.ticker % lightOverrideRefreshRate == 0 && mc.theWorld != null)
		{
			...
			for(Object obj : mc.theWorld.loadedEntityList)
			{
				...
				else if(obj instanceof EntityGrenade)
				{
					EntityGrenade grenade = (EntityGrenade)obj;
					//IDynamicLightSource iDynamicLightSource = new AddFlansLightSource((Entity)bullet,15);
					if(!grenade.isDead && grenade.type.hasLight)
					{
						int x = MathHelper.floor_double(grenade.posX);
						int y = MathHelper.floor_double(grenade.posY);
						int z = MathHelper.floor_double(grenade.posZ);
						blockLightOverrides.add(new Vector3i(x, y, z));
						mc.theWorld.setLightValue(EnumSkyBlock.Block, x, y, z, 15);
						mc.theWorld.updateLightByType(EnumSkyBlock.Block, x, y + 1, z);
						mc.theWorld.updateLightByType(EnumSkyBlock.Block, x, y - 1, z);
						...
						mc.theWorld.updateLightByType(EnumSkyBlock.Block, x, y, z - 1);
						//DynamicLights.addLightSource(iDynamicLightSource);
					}else if(grenade.isDead && grenade.type.hasLight){
						//DynamicLights.removeLightSource(iDynamicLightSource);
					}
				}
	...
	public void renderTickEnd(Minecraft mc)
	{
		...
		if(isInFlash && tickcountflash < flashTime){
			...
			GL11.glDepthMask(false);
			GL11.glBlendFunc(770, 771);
			GL11.glColor4f(1.0F, 1.0F, 1.0F, (float) (  ( (float)-tickcountflash ) / ( Math.max(1, (float)flashTime*0.2) ) +5  ));
			GL11.glDisable(3008 /* GL_ALPHA_TEST */);
