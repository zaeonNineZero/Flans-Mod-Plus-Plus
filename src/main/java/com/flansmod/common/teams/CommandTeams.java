	...
	public void processCommand(ICommandSender sender, String[] split)
	{
		...
		*/
		...
		if(split[0].equals("explosions"))
		{
			...
			sender.addChatMessage(new ChatComponentText("Explosion block damage is now " + (TeamsManager.explosions ? "enabled" : "disabled")));
			return;
		}
		...
		if(split[0].equals("canBreakGlass"))
		{
			...
			sender.addChatMessage(new ChatComponentText("Glass and glowstone can " + (TeamsManager.canBreakGlass ? "now" : "no longer") + " be broken by bullets"));
			return;
		}
