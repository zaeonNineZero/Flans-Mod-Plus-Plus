...
public class EntityMG extends Entity implements IEntityAdditionalSpawnData
{
	...
	public void onUpdate()
	{
		...
		if(!worldObj.isRemote && isShooting)
		{
			...
			worldObj.spawnEntityInWorld(((ItemBullet)ammo.getItem()).getEntity(worldObj, Vec3.createVectorHelper(blockX + 0.5D, blockY + type.pivotHeight, blockZ + 0.5D), (direction * 90F + rotationYaw), rotationPitch, gunner, type.bulletSpread, type.damage, ammo.getItemDamage(), type, bullet.doKnockback, bullet.modifyKnockback, bullet.kbDampener));
			if (soundDelay <= 0)
			{
	...
	public boolean attackEntityFrom(DamageSource damagesource, float i)
	{
		if (damagesource.damageType.equals("player"))
		{
			...
			if (player == gunner)
			{
				...
				if (!worldObj.isRemote)
				{
					worldObj.spawnEntityInWorld(((ItemBullet)ammo.getItem()).getEntity(worldObj, (EntityLivingBase) player, type.bulletSpread, type.damage, type.bulletSpeed, false, ammo.getItemDamage(), type, bullet.doKnockback, bullet.modifyKnockback, bullet.kbDampener));
				}
				if (soundDelay <= 0)
				{
