...
public class GrenadeType extends ShootableType
{
	...
	public boolean renderItemModel = true;
	...
	/** The max distance of the smoke particles; defined as flakDistance for continuity */
	public float flakDistance = 5F;
	/** The density of the smoke; determines how many particles spawn per tick */
	public int smokeDensity = 1;
	...
	protected void read(String[] split, TypeFile file)
	{
		...
		try
		{
			...
			if(split[0].equals("RenderFull3D"))
				renderFull3D = Boolean.parseBoolean(split[1]);
			if(split[0].equals("RenderItemModel"))
				renderItemModel = Boolean.parseBoolean(split[1]);
			...
			else if(split[0].equals("SmokeParticleDistance"))
				flakDistance = Float.parseFloat(split[1]);
			else if(split[0].equals("SmokeDensity"))
				smokeDensity = Integer.parseInt(split[1]);
