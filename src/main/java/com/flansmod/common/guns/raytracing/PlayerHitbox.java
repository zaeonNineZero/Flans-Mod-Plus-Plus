...
public class PlayerHitbox 
{
	...
	public float hitByBullet(EntityBullet bullet, float penetratingPower) 
	{
		...
		case LEFTARM : damageModifier *= 0.8F; break;
		case RIGHTARM : damageModifier *= 0.8F; break;
		case LEFTITEM : break;
		...
		case BODY :  case HEAD :  case LEFTARM :  case RIGHTARM : 
		{
			...
			if(player.attackEntityFrom(damagesource, hitDamage))
			{
				...
				*/
				double kbMotionX = player.motionX;
				double kbMotionY = player.motionY;
				double kbMotionZ = player.motionZ;
				
				if (bullet.modifyKnockback == true)
				{
					player.motionX = (kbMotionX + (velocityX*Math.max(0,bullet.kbDampener)))/(1+bullet.kbDampener);
