...
public class ItemGun extends Item implements IFlanItem
{
	...
	public void onUpdateClient(ItemStack itemstack, World world, Entity entity, int i, boolean flag)
	{
		if(entity instanceof EntityPlayer && ((EntityPlayer)entity).inventory.getCurrentItem() == itemstack)
		{
			...
			else
			{
				...
				{
					if(FlansModClient.currentScope == null && (data.reloadTime <= 0))
					{
						...
						gameSettings.mouseSensitivity = f / (float) Math.sqrt((currentScope.getZoomFactor())+(currentScope.getFOVFactor()));
					...
					FlansModClient.scopeTime = 13;
				}
				
				if ((data.reloadTime > 0) && FlansModClient.currentScope != null)
				{
					//if(type.allowNightVision)
						//isNightVision = false;
					FlansModClient.currentScope = null;
	...
	public boolean clientSideShoot(EntityPlayer player, ItemStack stack, GunType gunType, boolean left)
	{
		...
		if(FlansModClient.shootTime(left) <= 0 && (type.canSprintShoot || !player.isSprinting()/* || ((left && data.burstRoundsRemainingLeft > 0) || *(!left && data.burstRoundsRemainingRight > 0))*/))
		{
			...
			if(hasAmmo)
			{
				...
				if(!player.isSneaking())
				{
					...
					if (gunType.decreaseRecoilDivider)
					FlansModClient.playerRecoilPitch += gunType.getRecoilPitch(stack) / gunType.getDecreaseRecoilPitch(stack);
					else
					FlansModClient.playerRecoilPitch += gunType.getRecoilPitch(stack) - gunType.getDecreaseRecoilPitch(stack);
			...
			if(gunType.getFireMode(stack) == EnumFireMode.BURST)
			{
				if(left)
				{
					...
					else if (type.canSprintShoot || !player.isSprinting())
	...
	public void onMouseHeld(ItemStack stack, World world, EntityPlayerMP player, boolean left, boolean isShooting)
	{
		...
		if(data != null && data.shootClickDelay == 0)
		{
			...
			if(left && data.offHandGunSlot != 0)
			{
				...
				if(gunType.getFireMode(offHandGunStack) == EnumFireMode.BURST && isShooting && data.burstRoundsRemainingLeft == 0 && (type.canSprintShoot || !player.isSprinting()))
				{
					data.isShootingLeft = false;
					data.burstRoundsRemainingLeft = gunType.numBurstRounds;
					player.inventory.setInventorySlotContents(data.offHandGunSlot - 1, tryToShoot(offHandGunStack, gunType, world, player, true));
			...
			else
			{
				...
				if(type.getFireMode(stack) == EnumFireMode.BURST && isShooting && data.burstRoundsRemainingRight == 0 && (type.canSprintShoot || !player.isSprinting()))
				{
					data.isShootingRight = false;
	...
	public ItemStack tryToShoot(ItemStack gunStack, GunType gunType, World world, EntityPlayerMP entityplayer, boolean left)
	{
		...
		if( ((left && data.shootTimeLeft <= 0) || (!left && data.shootTimeRight <= 0)) && (type.canSprintShoot || !entityplayer.isSprinting() || gunType.getFireMode(gunStack) == EnumFireMode.BURST && ((left && data.burstRoundsRemainingLeft > 0) || (!left && data.burstRoundsRemainingRight > 0))) )
		{
			...
			if(bulletStack == null)
			{
				if(reload(gunStack, gunType, world, entityplayer, false, left))
				{
					...
					data.reloadTime = (int)gunType.getReloadTime(gunStack);
					...
					if(gunType.reloadSound != null)
						PacketPlaySound.sendSoundPacket(entityplayer.posX, entityplayer.posY, entityplayer.posZ, type.reloadSoundRange, entityplayer.dimension, gunType.reloadSound, false);
				}
			}
			...
			else if(bulletStack.getItem() instanceof ItemShootable)
			{
				...
				//Check if we are out of ammo, and apply a delay before we can reload
				boolean hasAmmo = false;
				for(int i = 0; i < gunType.numAmmoItemsInGun; i++)
				{
					ItemStack checkingStack = getBulletItemStack(gunStack, i);
					if(checkingStack != null && checkingStack.getItem() != null && checkingStack.getItemDamage() < checkingStack.getMaxDamage())
					{
						hasAmmo = true;
						break;
				...
				if (!hasAmmo && gunType.reloadDelay > gunType.shootDelay)
				{
					if (left)
					{
						data.shootTimeLeft = gunType.reloadDelay;
					}
					else
					{
						data.shootTimeRight = gunType.reloadDelay;
					}
				}
	...
	private void shoot(ItemStack stack, GunType gunType, World world, ItemStack bulletStack, EntityPlayer entityplayer, boolean left)
	{
		...
		if (soundDelay <= 0 && gunType.shootSound != null)
		{
			...
			if (barrel != null && gunType.customSilencedSound == true)
			{
				if (barrel.silencer)
				{
					PacketPlaySound.sendSoundPacket(entityplayer.posX, entityplayer.posY, entityplayer.posZ, (type.gunSoundRange / 2), entityplayer.dimension, gunType.shootSoundSilenced, gunType.distortSound, false);
				}
				else
				{
			...
			PacketPlaySound.sendSoundPacket(entityplayer.posX, entityplayer.posY, entityplayer.posZ, type.gunSoundRange, entityplayer.dimension, bullet.fireSound, gunType.distortSound, silenced);
		...
		if (!world.isRemote && bulletStack.getItem() instanceof ItemShootable)
		{
			...
			boolean shootDoKnockback   	 = gunType.doKnockback;
			boolean shootModifyKnockback = gunType.modifyKnockback;
			double  shootkbDampener 	 = gunType.kbDampener;
			...
			if(shootableType instanceof BulletType)
			{
				...
				if(gunType.allowBulletSpeedByBulletType)
				{
					speed = ((BulletType) shootableType).bulletSpeed;
				}
			...
			if (gunType.useBulletKnockback == true)
			{
				shootDoKnockback   	 = shootableType.doKnockback;
				shootModifyKnockback = shootableType.modifyKnockback;
				shootkbDampener 	 = shootableType.kbDampener;
			}
			...
			for (int k = 0; k < numBullets; k++)
			{
				world.spawnEntityInWorld(itemShootable.getEntity(
						world,
						entityplayer,
						(entityplayer.isSprinting() ? gunType.spreadSprintMultiplier : 1F) * (isScoped ? gunType.spreadAimMultiplier : gunType.spreadHipfireMultiplier) * spread,
						gunType.getDamage(stack),
						speed,
						numBullets > 1,
						bulletStack.getItemDamage(),
						gunType, shootDoKnockback, shootModifyKnockback, shootkbDampener));
			}
	...
	public static String getPaintTag(ItemStack stack)
    {
		NBTTagCompound tag = stack.stackTagCompound;
		return tag.getString("Paint");
    }
    ...
    public Multimap getAttributeModifiers(ItemStack stack)
    {
        ...
        if(type.secondaryFunction == EnumSecondaryFunction.MELEE)
        	map.put(SharedMonsterAttributes.attackDamage.getAttributeUnlocalizedName(), new AttributeModifier(field_111210_e, "Weapon modifier", type.getMeleeDamage(stack), 0));
       	return map;
    }
