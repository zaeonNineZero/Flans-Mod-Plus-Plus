...
public class AttachmentType extends InfoType implements IScope
{
	...
	/** This variable controls whether or not a barrel should override the muzzle flash */
	public boolean overrideMuzzleSettings = false;
	public boolean useMuzzleParticle = false;
	public String muzzleParticleType = "largesmoke";
	
	/** Determines if this attachment is a bipod, which in turn enables usage of the bipodMultiplier */
	...
	/** If this attachment is a bipod, this determines the recoilDecrease multiplier. */
	public float bipodMultiplier = 1F;
	...
	public String zoomOverlay = "None";
	...
	protected void read(String[] split, TypeFile file)
	{
		...
		try
		{
			...
			else if(split[0].equals("Bipod"))
				bipod = Boolean.parseBoolean(split[1].toLowerCase());
			
			else if(split[0].equals("OverrideMuzzleSettings"))
				overrideMuzzleSettings = Boolean.parseBoolean(split[1].toLowerCase());
			else if(split[0].equals("UseMuzzleParticle"))
				useMuzzleParticle = Boolean.parseBoolean(split[1].toLowerCase());
			else if(split[0].equals("MuzzleParticleType"))
				muzzleParticleType = split[1];
			...
			else if(split[0].equals("BipodEffectivenessMultiplier"))
			{
				if (bipod == true)
				bipodMultiplier = Float.parseFloat(split[1]);
			}
			//Scope Variables
