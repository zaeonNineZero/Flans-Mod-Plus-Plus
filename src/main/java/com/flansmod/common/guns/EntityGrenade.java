...
public class EntityGrenade extends EntityShootable implements IEntityAdditionalSpawnData
{
	...
	public EntityGrenade(World w, GrenadeType g, EntityLivingBase t)
	{
		...
		if(type.throwSound != null)
			worldObj.playSoundEffect(posX, posY, posZ, "flansmod:" + type.throwSound, 1.1F, (float) ((Math.random()*0.1F)+0.9F));
			//PacketPlaySound.sendSoundPacket(posX, posY, posZ, FlansMod.soundRange, dimension, type.throwSound, false);
	}
	...
	public void onUpdate()
	{
		...
		if(smoking)
		{
			...
			//We are repeating this twice for smoke density
			for (int i = 0; i < type.smokeDensity; i++)
			{
				FlansMod.getPacketHandler().sendToAllAround(new PacketFlak(posX, posY, posZ, 50, type.smokeParticleType, type.flakDistance), posX, posY, posZ, 30, dimension);
			}
			//
		...
		if(!stuck && !type.stickToThrower)
		{
			...
			if(hit != null && hit.typeOfHit == MovingObjectType.BLOCK)
			{
				...
				else if(!type.penetratesBlocks)
				{
					...
					if(motVec.lengthSquared() > 0.002D)
						playSound("flansmod:" + type.bounceSound, 1.0F, 1.2F / (this.rand.nextFloat() * 0.2F + 0.9F));
	...
	public boolean interactFirst(EntityPlayer player)
	{
		...
		if(type.isDeployableBag && !worldObj.isRemote)
		{
			...
			if(type.healAmount > 0 && player.getHealth() < player.getMaxHealth())
			{
	        	...
	        	FlansMod.getPacketHandler().sendToAllAround(new PacketFlak(player.posX, player.posY, player.posZ, 5, "heart", 1F), new NetworkRegistry.TargetPoint(player.dimension, player.posX, player.posY, player.posZ, 50F));
	        	used = true;
			}
