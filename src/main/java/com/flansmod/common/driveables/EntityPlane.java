...
public class EntityPlane extends EntityDriveable
{
		...
		switch(key)
		{
			case 0 : //Accelerate : Increase the throttle, up to 1.
			{
				if(canThrust || throttle < 0F)
				{
					...
					if(throttle > 1F)
						throttle = 1F;
					if(mode == EnumPlaneMode.HELI && throttle < 0.25F)
						throttle += 0.005F;
				}
				return true;
			}
			...
			{
				if((throttle > 0F && mode != EnumPlaneMode.HELI) || (throttle > 0.25F))
				{
					throttle -= 0.006F;
					if(throttle < -1F)
						throttle = -1F;
				...
				if(toggleTimer <= 0)
				{
					...
					if(type.mode == EnumPlaneMode.VTOL)
					{
						...
						else
						{
							mode = EnumPlaneMode.HELI;
							planeMode = "Heli";
							if (throttle >= 0.25F && throttle < 0.48F)
							throttle = 0.48F;
							if (throttle > 0.5F)
							throttle = 0.5F;
							trim = false;
						}
						player.addChatMessage(new ChatComponentText(mode == EnumPlaneMode.HELI ? "Entering Hover Mode" : "Entering Flight Mode"));
					}
					toggleTimer = 10;
					FlansMod.getPacketHandler().sendToServer(new PacketDriveableControl(this));
				}
            ...
            case 16 : // Trim Button
            {
                if(type.mode == EnumPlaneMode.PLANE)
				{
					if(trim == false)
					trim = true;
					else trim = false;
					player.addChatMessage(new ChatComponentText(trim ? "Airbrakes Enabled" : "Airbrakes Disabled"));
				}
				return true;
            }
            case 17 : //Park
	...
	public void onUpdate()
    {
		...
		/* Glidespeed handling for planes; override to equal throttle for helicopters and hovering VTOLs */
		if(mode == EnumPlaneMode.PLANE)
		{
		float glidespeedDrag = 0.0005F;
		
		//Match throttle if throttle goes negative
		if(throttle < 0)
		glidespeed = (canThrust ? throttle : 0);
		else
		{
		//Throttle provides thrust,
		if(glidespeed <= throttle && canThrust)
		glidespeed = throttle;
		else
		{
			glidespeed = ((glidespeed*9)+(throttle*1))/10;
			if (glidespeed > throttle || !canThrust)
			glidespeed -= glidespeedDrag;
			
			//Repeat throttle check when fueled
			if(glidespeed < throttle && canThrust)
			glidespeed = throttle;
			
			//Zero glidespeed when it's low enough
			if(Math.abs(glidespeed) < glidespeedDrag*2)
			glidespeed = 0;
		}
		//Reduce the increase in flight speed at high throttle values
		float softcapAirspeed = type.maxAirspeed * 0.75F;
		if(glidespeed > softcapAirspeed)
		glidespeed -= (glidespeed - softcapAirspeed)/4;
		
		//Hardcap for glide speed
		if(glidespeed > type.maxAirspeed)
		glidespeed = type.maxAirspeed;
		...
		//With a player, and once throttled up enough, default to 0.5 for helicopters (hover speed)
		...
		if(seats[0] != null && seats[0].riddenByEntity != null && mode == EnumPlaneMode.HELI && canThrust)
		{
			...
			throttle -= Math.min(0.005,throttle);
		}
		
		//If out of fuel, slowly reduce throttle
		if(!canThrust)
		{
			if (throttle < 0)
			throttle += 0.005F;
			...
			if (throttle < 0.005 && throttle > -0.005)
			throttle = 0;
		}
		...
		float sensitivityAdjust = (glidespeed > 0.5F ? 1.5F - glidespeed : 4F * glidespeed - 1F);
		...
		case HELI :
			...
			Vector3f up = (Vector3f)axes.getYAxis().normalise();
			...
			lastPos = new Vector3f(motionX, motionY, motionZ);
			
			if(!canThrustCreatively && throttle>0)
		...
		case PLANE :
			...
			float proportionOfMotionToCorrect = 2F * glidespeed - 0.5F;
			...
			float amountOfLift = type.lift * g * glidespeed * numWingsIntact / 2F;
			if(amountOfLift > g)
				amountOfLift = g;

			if(isPartIntact(EnumDriveablePart.rightWing) || isPartIntact(EnumDriveablePart.leftWing))
			...
			if(!isPartIntact(EnumDriveablePart.rightWing) && !isPartIntact(EnumDriveablePart.rightWing)){
				motionY += -1;
			}
