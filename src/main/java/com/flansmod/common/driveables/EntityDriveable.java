...
public abstract class EntityDriveable extends Entity implements IControllable, IExplodeable, IEntityAdditionalSpawnData
{
	...
	/** Previous position values, for calculating speed for use with TickHandlerClient */
	public double pPosX, pPosY, pPosZ;
	...
	public float adjustThrottle = 0;
	/** This variable controls gliding airspeed for aircraft in plane mode */
	...
	public void readSpawnData(ByteBuf data)
	{
		...
		{
			FlansMod.log("Failed to retrieve driveable type from server.");
			super.setDead();
			e.printStackTrace();
		}
...
//				damagesource.getDamageType(), damagesource.getEntity(), damagesource.getSourceOfDamage()));
		...
		//Ignore cactus damage
		if (damagesource.getDamageType().indexOf("cactus") < 0)
		{
		...
		if(i > 0)
		{
			if (dmgSoundCount <= 0)
			{
				worldObj.playSoundEffect(posX, posY, posZ, "flansmod:vehiclehit", 1.1F, (float) ((Math.random()*0.2F)+0.9F));
				dmgSoundCount = 10;
			}
		...
		if(shootPoint.rootPos instanceof PilotGun)
		{
			...
			if(gunType != null && bulletItemStack != null && bulletItemStack.getItem() instanceof ItemShootable && TeamsManager.bulletsEnabled)
			{
				...
				if(gunType.isAmmo(bullet))
				{
					...
					worldObj.spawnEntityInWorld(((ItemShootable)bulletItemStack.getItem()).getEntity(worldObj, Vector3f.add(new Vector3f(posX, posY, posZ), gunVec, null), lookVector, (EntityLivingBase)seats[0].riddenByEntity, gunType.bulletSpread / 2, gunType.damage, shellSpeed,bulletItemStack.getItemDamage(), type, bullet.doKnockback, bullet.modifyKnockback, bullet.kbDampener));
			...
			case BOMB :
			{
				if(TeamsManager.bombsEnabled)
				{
					...
					if(slot != -1)
					{
						...
						EntityShootable bulletEntity = bulletItem.getEntity(worldObj, Vec3.createVectorHelper(gunVec.x + posX, gunVec.y + posY, gunVec.z + posZ), axes.getYaw(), axes.getPitch(), motionX, motionY, motionZ, (EntityLivingBase)seats[0].riddenByEntity, damageMultiplier, driveableData.getStackInSlot(slot).getItemDamage(), type, bullet.doKnockback, bullet.modifyKnockback, bullet.kbDampener);
			...
			case SHELL :
			{
				...
				if(TeamsManager.shellsEnabled)
				{
					...
					if(slot != -1)
					{
						...
						ItemBullet bulletItem = (ItemBullet)bulletStack.getItem();
						ShootableType bullet = ((ItemShootable)bulletStack.getItem()).type;
						EntityShootable bulletEntity = bulletItem.getEntity(worldObj, Vector3f.add(gunVec, new Vector3f(posX, posY, posZ), null), lookVector, (EntityLivingBase)seats[0].riddenByEntity, spread, damageMultiplier, shellSpeed, driveableData.getStackInSlot(slot).getItemDamage(), type, bullet.doKnockback, bullet.modifyKnockback, bullet.kbDampener);
    ...
    public void onUpdate()
    {
		...
		adjustThrottle = (adjustThrottle*6 + throttle)/7;
		...
		if(throttle != 0)
		{
			propAngle += (Math.pow(throttle, 0.4))*1.6;	
			rotorAngle += throttle/6F;
		}
        ...
        for(int i = 0; i < type.emitters.size(); i++)
        {
			...
			if(this instanceof EntityVehicle){
				if (emitter.mode.equals("Boost") && isBoosting == true)
				properMode = true;
				else
				if (emitter.mode.equals("NoBoost") && isBoosting == false)
				properMode = true;
			}
			else
			properMode = true;
...
// on first update
		...
		for(int i = 0; i < data.getSizeInventory(); i++)
		{
			...
			if(data.engine.useRFPower)
			{
				if(item instanceof IEnergyContainerItem)
				{
					...
					data.fuelInTank += (fuelMultiplier/2 * energy.extractEnergy(stack, data.engine.RFDrawRate, false)) / data.engine.RFDrawRate;
				}
			}
			else
			{
				...
				if(item instanceof ItemPart)
				{
					...
					if(part.category == 9 && (type.canRefuelInMotion || Math.round(throttle*100)==0))
					{
						...
						{
						//Repeat this part to speed up fueling
						for(int r = 0; r < 2 && data.fuelInTank < type.fuelTankSize; r++){
						...
						{
							ItemStack emptyStack = getEmptyFuelStack(part.emptyItem);
							
							//Reset the damage to 0
							stack.setItemDamage(0);
							//Consume one item
							stack.stackSize--;
							//If we consumed the last one, destroy the stack if emptyItem is null 
							if(stack.stackSize <= 0 && part.emptyItem == null)
							{
								data.setInventorySlotContents(i, null);
								break;
							}
							else //Otherwise, replace the stack with one empty container.  ONLY WORKS WITH SINGLE-STACK CONTAINERS
							if(stack.stackSize <= 0 && part.emptyItem != null)
							{
								stack.stackSize = 1;
								data.setInventorySlotContents(i, emptyStack);
								break;
							}
						}}
	...
	private ItemStack getEmptyFuelStack(String itemName) {
		if (itemName != null)
		{
			int damage = 0;
			...
			ItemStack item = InfoType.getRecipeElement(itemName, damage);
			return item;
		}
		else
		return null;
	...
	public void checkForCollisions()
	{
		...
		for(DriveablePosition p : getDriveableType().collisionPoints)
		{
			...
			if(hit != null && hit.typeOfHit == MovingObjectType.BLOCK)
			{
				...
				float damage = blockHardness * 3 * (float)speed;
				if(blockHardness>=0.5)
				{
					if(this instanceof EntityPlane)
					{
						damage += 10;
				...
				//Attack the part
				if(!attackPart(p.part, DamageSource.inWall, damage))
				{
					//And if it didn't die from the attack, break the block
					if(blockHardness>=0.5 || TeamsManager.driveablesBreakBlocks)
					...
					if(!worldObj.isRemote && TeamsManager.driveablesBreakBlocks)
					{
...
//				checkParts();
	...
	/** Alternative way to get a vehicle's motion, for use with TickHandlerClient */
	public double getMotion(String checkMode)
	{
		Vector3f motionV = new Vector3f(posX - pPosX, posY - pPosY, posZ - pPosZ);
		...
		if (checkMode=="xz")
		return Math.sqrt((motionV.x * motionV.x) + (motionV.z * motionV.z));
		if (checkMode=="xyz")
		return Math.sqrt((motionV.x * motionV.x) + (motionV.y * motionV.y) + (motionV.z * motionV.z));
	...
	/** Method for returning empty fuel items */
	public static void returnEmptyItem(DriveableData data, int fuelSlot, String itemName)
	{
		if (itemName != null)
		{
			int damage = 0;
			...
			ItemStack emptyStack = InfoType.getRecipeElement(itemName, damage);
			data.setInventorySlotContents(fuelSlot, emptyStack);
		}
	}
