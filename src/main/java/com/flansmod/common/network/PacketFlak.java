...
public class PacketFlak extends PacketBase 
{
	...
	/** Particle distance */
	public float flakDistance;

	public PacketFlak() {}

	public PacketFlak(double x1, double y1, double z1, int n, String s, float d)
	{
		x = x1; y = y1; z = z1;
		numParticles = n;
		particleType = s;
	...
	public void handleClientSide(EntityPlayer clientPlayer) 
	{
		...
		{
			EntityFX obj = FlansModClient.getParticle(particleType, world, x + (rand.nextGaussian()*(flakDistance/4)), y + (rand.nextGaussian()*(flakDistance/4)), z + (rand.nextGaussian()*(flakDistance/4)));
			if(obj != null)
			{
